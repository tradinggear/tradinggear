    ...
    const fetchData = async () => {
      ...
      if(data1[0]["trading_center"] == "kium") {
        ...
        */
        ...
        const res = await fetch("https://tradinggear.co.kr:777/api/fapi/daily_change", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        //console.log(text3.totals.unRealizedProfit);
        //console.log(text3.totals.roi_pct);        
        ...
        const recentTradesArrayData = text5.recentTradesArray;
        //console.log(text5.recentTradesArray);

        let portfolioArray: any = [];
        //let recentTradesArray: any = [];        

        const arr = Array.isArray(text5?.recentTradesArray) ? text5.recentTradesArray : [];

        const recentTradesArray = arr.map((item: any) => ({
          id: item.id,
          symbol: item.symbol,
          type: item.type,
          price: item.price,
          ...
          profitRate:
            item.profitRate === null || item.profitRate === undefined || item.profitRate === ''
              ? null
        ...
        const newArray = js.data.map((d: any) => ({ time: d.time, value: d.value }));
        console.log(newArray);
        setChartDataByPeriod({ 차트: newArray });
        ...
        const portfolio = await res7.json(); // 배열
