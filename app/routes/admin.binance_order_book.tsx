...
const AdminDashboard = () => {
  ...
  const [priceSum2, setPriceSum2] = useState<number>(0);
    ...
    const fetchOrderBook = async () => {
      try {
        ...
        const sorted = [...data.bids].sort(
          (a: [string, string], b: [string, string]) => parseFloat(b[1]) - parseFloat(a[1])
        );
        setBids(sorted);
        // bids 배열의 가격만 누적 합계
        //const priceSum = data.bids.reduce((acc: number, [price]) => {
        //  return acc + parseFloat(price);
        //}, 0); 
        
        // 수량만 누적
        const qtySum = data.bids.reduce((acc: number, [, qty]) => acc + parseFloat(qty), 0);
        //setQtySum(qtySum);        
  ...
  useEffect(() => {
    const fetchOrderBook = async () => {
      try {
        ...
        setAsks(sorted);
        // bids 배열의 가격만 누적 합계
        //const priceSum2 = data.asks.reduce((acc: number, [price]) => {
        //return acc + parseFloat(price);
        //}, 0);   
        //console.log("priceSum2="+priceSum2);        
        // 수량만 누적
        const qtySum = data.asks.reduce((acc: number, [, qty]) => acc + parseFloat(qty), 0);        
...
//}
  ...
  const menuItems = [
    { icon: Users, label: '회원목록', href: '/admin/member', active: false },
    { icon: Users, label: '바이낸스오더북', href: '/admin/binance_order_book', active: true }, 
      ...
      `}>
        ...
        <main className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className={`text-2xl font-bold ${themeClasses.text} mb-2`}>바이낸스(SOLUSDT) 오더북 관리</h2>
              {/*<p className={themeClasses.textMuted}>총 {membersData.length}명의 회원이 있습니다.</p>*/}
            </div>
          </div>
    {/*<div>
      <h2>SOLUSDT 오더북 가격 합계</h2>
      <p>🟢 Bids 가격 합계: {priceSum.toFixed(2)}</p>
      <p>🔴 Asks 가격 합계: {priceSum2.toFixed(2)}</p>
      <p>📊 총합 (Bids + Asks): {(priceSum + priceSum2).toFixed(2)}</p>
    </div>*/}
    <h1 style={{color:"white"}}>CVD: {cvd.toFixed(4)}</h1>
    {(priceSum - priceSum2 > 0) ? <div className='flex items-center' style={{color:"white"}}>매수세가 강합니다.{(priceSum - priceSum2).toFixed(2)}</div> : <div className='flex items-center' style={{color:"white"}}>매도세가 강합니다.{(priceSum - priceSum2).toFixed(2)}</div>}
          {/* 검색 영역 */}
          {/*
              ...
              <table className="w-auto">
                <thead>
                  <tr className={`border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} ${themeClasses.tableHeader}`} >
                    ...
                    >
                      <div className="flex items-center" style={{ color: "blue" }}>
                        PRICE(BIDS)
                        {getSortIcon('email')}
                      </div>
                    </th>
                ...
                <tbody>
                  {/* {sortedMembers.map((member, index) => (*/}
                    ...
                    <tr key={price} className={`border-b ${themeClasses.tableRow} transition-colors duration-200`}>
                      <td className={`py-4 px-6 ${themeClasses.textSecondary}`} style={{ backgroundColor: qty > 100 ? "gray" : "" }}>{price}</td>
                      <td className={`py-4 px-6 text-center ${themeClasses.text} font-medium`} style={{ backgroundColor: qty > 100 ? "gray" : "" }}>{qty}</td>
                      {/*
