    ...
    <header className={`fixed top-0 w-full backdrop-blur-lg z-50 border-b transition-all duration-300 ${headerClasses}`}>
      <nav className="max-w-7xl mx-auto flex justify-between items-center px-4 lg:px-8 py-4">
        ...
        <ul className="hidden lg:flex items-center space-x-8">
          <li><a href="/about" className={`${textPrimary} hover:${primaryColor.replace('text-', 'text-')} font-medium cursor-pointer transition-colors duration-300`}>회사소개</a></li>
      ...
      {/* Mobile Sidebar Menu */}
      <div className={`lg:hidden fixed top-0 right-0 h-screen w-80 max-w-[80vw] ${theme === 'dark' ? 'bg-slate-900/95' : 'bg-white/95'} backdrop-blur-lg border-l ${theme === 'dark' ? 'border-cyan-400/20' : 'border-blue-600/20'} z-50 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>

        {/* Mobile Menu Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-200/20">
          <div className={`flex items-center text-xl font-bold ${primaryColor}`}>
            <a href="/" className={`${textPrimary} hover:${primaryColor.replace('text-', 'text-')} text-sm font-medium cursor-pointer transition-colors duration-300`}>Home</a>
          </div>
          <button
        ...
        <div className={`flex flex-col h-[calc(100vh-80px)]`}>
          <ul className="px-6 py-8 space-y-6 flex-1">
            <li><a href="/about" className={`block ${textPrimary} hover:${primaryColor.replace('text-', 'text-')} font-medium cursor-pointer transition-colors duration-300 py-3 text-lg border-b border-gray-200/10`} >회사소개</a></li>
            <li><a href="/blog" className={`block ${textPrimary} hover:${primaryColor.replace('text-', 'text-')} font-medium cursor-pointer transition-colors duration-300 py-3 text-lg border-b border-gray-200/10`} >블로그</a></li>
            <li><a href="/doc" className={`block ${textPrimary} hover:${primaryColor.replace('text-', 'text-')} font-medium cursor-pointer transition-colors duration-300 py-3 text-lg border-b border-gray-200/10`} >문서</a></li>
            <li><a href="/pricing" className={`block ${textPrimary} hover:${primaryColor.replace('text-', 'text-')} font-medium cursor-pointer transition-colors duration-300 py-3 text-lg border-b border-gray-200/10`} >요금제</a></li>
            <li><a href="/review" className={`block ${textPrimary} hover:${primaryColor.replace('text-', 'text-')} font-medium cursor-pointer transition-colors duration-300 py-3 text-lg border-b border-gray-200/10`} >리뷰</a></li>
          </ul>
